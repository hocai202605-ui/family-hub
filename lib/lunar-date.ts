/**
 * Vietnamese lunar calendar (âm lịch) helpers.
 * Algorithm adapted from the well-known Ho Ngoc Duc solar↔lunar conversion.
 */
...
/** Convert solar date to Vietnamese lunar date (timezone +7). */
...
/** Parse `YYYY-MM-DD` → lunar label for calendar cells (e.g. "15" or "1/8"). */
export function lunarLabelFromDateKey(dateKey: string): string {
  const [y, m, d] = dateKey.split("-").map(Number);
  if (!y || !m || !d) return "";
  const lunar = solarToLunar(d, m, y);
  if (lunar.day === 1) {
    return `${lunar.day}/${lunar.month}${lunar.leap ? "+" : ""}`;
  }
  return String(lunar.day);
}

export function lunarFullLabelFromDateKey(dateKey: string): string {
  const [y, m, d] = dateKey.split("-").map(Number);
  if (!y || !m || !d) return "";
  const lunar = solarToLunar(d, m, y);
  return `${lunar.day}/${lunar.month}${lunar.leap ? " (nhuận)" : ""} ÂL · năm ${lunar.year}`;
}
